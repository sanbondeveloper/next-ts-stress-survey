...
- 팀과 이름이 모두 입력되었을 경우에만 버튼을 활성화하기 위해 제어 컴포넌트 방식을 사용했습니다.
  - 시간이 된다면 개선할 점 2가지가 있습니다. 첫 번째는 선택할 수 있는 팀 데이터를 JSON 파일로 가져와서 Select 방식으로 구현하기 입니다. 두 번째는 이름의 길이를 최소 2자, 최대 10자로 제한하기 입니다.
...
- 화면에 전체 질문을 모두 렌더링하지만 CSS를 활용해 하나의 질문만 보여지도록 구현했습니다.
  - 하나의 문제가 폼 전체를 자치하게 하고 `transform: translateX(-${step * 100}%)`으로 x축을 수평 이동시켰습니다.
  - 각 step별로 하나의 컴포넌트만 렌더링할지 전체 질문을 렌더링하고 CSS를 통해 하나의 질문만 보여지도록 할지 고민했습니다. 전자의 방식은 화면이 깜빡이는 현상이 발생할 수도 있어 후자의 방식을 선택했습니다.
